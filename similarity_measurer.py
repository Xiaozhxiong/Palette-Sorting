...
from multiple_palettes_sorter import *
from color_palettes import *


class SimilarityMeasurer:
    def __init__(self, source_palette, target_palette, lab_distance_mode, improved=False, need_sort=True):
        self.source_palette = source_palette
        self.target_palette = target_palette

        self.color_palettes = ColorPalettes(auto_fetched=False, color_palettes_list=[source_palette, target_palette],
                                            is_hex_list=False)
        # 排序后再计算相似度
        if need_sort:
            multiple_palettes_sorter = MultiplePalettesSorter(self.color_palettes, 2, lab_distance_mode)
            self.sorted_palette_indices, _, _ = multiple_palettes_sorter.standard_sort(improved=improved)
        # 未排序计算相似度
        else:
            sp = self.source_palette.get_colors()
            tp = self.target_palette.get_colors()
            self.sorted_palette_indices = [sp, tp]
        # print('=======>',self.sorted_palette_indices)
    ...
    def _measure_pairwise_average(self):
        ...
        similarity = sum(sum(x) for x in pairwise_distance_map) / (
                len(pairwise_distance_map) * len(pairwise_distance_map[0]))
    ...
    def _measure_merged_palette_histogram_similarity_measure(self):
        ...
        while self._get_Euclidean_distance_between_two_labs(closest_a_lab, closest_b_lab) <= Td:
            ...
            c_m_lab = LabColor(c_m[0], c_m[1], c_m[2], observer=closest_a_lab.observer,
                               illuminant=closest_a_lab.illuminant)
            common_palette.append(c_m_lab)

            if is_a_from_s:
                distance_map[closest_s_index, :] = np.iinfo(np.int64).max
                remaining_s_indices = np.delete(remaining_s_indices,
                                                np.argwhere(remaining_s_indices == closest_s_index))
            if is_b_from_t:
                distance_map[:, closest_t_index] = np.iinfo(np.int64).max
                remaining_t_indices = np.delete(remaining_t_indices,
                                                np.argwhere(remaining_t_indices == closest_t_index))

            if len(remaining_s_indices) == 0 and len(remaining_t_indices) == 0:
                break

            # (1) the closest between two descriptors
            st_closest_s_index, st_closest_t_index = np.unravel_index(np.argmin(distance_map, axis=None),
                                                                      distance_map.shape)
            ...
            if len(common_palette) > 0:
                if len(remaining_s_indices) > 0:
                    ...
                    scp_distance_map = np.array(
                        self._get_distance_map_between_two_lab_lists(remaining_s_labs, common_palette,
                                                                     use_euclidean=True))
                    scp_closest_s_index, scp_closest_cp_index = np.unravel_index(np.argmin(scp_distance_map, axis=None),
                                                                                 scp_distance_map.shape)
                ...
                if len(remaining_t_indices) > 0:
                    ...
                    tcp_distance_map = np.array(
                        self._get_distance_map_between_two_lab_lists(remaining_t_labs, common_palette,
                                                                     use_euclidean=True))
                    tcp_closest_t_index, tcp_closest_cp_index = np.unravel_index(np.argmin(tcp_distance_map, axis=None),
                                                                                 tcp_distance_map.shape)
