...
from single_palette_sorter import *
...
class MultiplePalettesSorter:
    ...
    def standard_sort(self, improved=False):
        if improved:
            start_time = time.time()
            hex_palette = self.palettes.to_hex_list()
            palette = hex_palette[0] + hex_palette[1]
            sorter = OrderSorter(palette=palette, name='', solve_mode=SolverMode.PSO)
            sorter.calculate_distance_matrix()
            step = 1
            sorter.discrete_sampling(step=step)
            best_points = sorter.sort()  # 排序
            sorter.solve(best_points)
            sorted_points = sorter.samples_sorted
            n = len(palette) // 2
            palette_1 = [x for x in sorted_points if x < n]
            palette_2 = [x - n for x in sorted_points if x >= n]
            sorted_result = [palette_1, palette_2]
            ...
            return self.sort(multiple_palette_sort_mode, merge_cut_type)
    ...
    def sort(self, multiple_palette_sort_mode, merge_cut_type=MergeCutType.With_Cutting):
        ...
        # print(multiple_palette_sort_mode)
        ...
        if multiple_palette_sort_mode == MultiplePalettesSortMode.Separate_Luminance:
            ...
            sorted_result, _, merged_sorted_indices = self._sort_merge(single_palette_standard_sort_mode,
                                                                       merge_cut_type)
    ...
    def _sort_merge(self, single_palette_sort_mode, merge_cut_type):
        ...
        for p_index in range(self.palette_count):
            ...
            sorted_result.append([i - color_start_index for i in merged_sorted_indices if
                                  i >= color_start_index and i < color_end_index])
    ...
    def _sort_minimum_bipartite_matching_error(self):
        ...
        sorted_basis_palette = ColorPalette(auto_fetched=False, palette_length=len(sorted_basis_hex_list),
                                            colors=sorted_basis_hex_list)
