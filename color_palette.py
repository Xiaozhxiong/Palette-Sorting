...
class ColorPalette:
    ...
    def bulk_up(self, width=100, add_white_line=False, repeat=True):
        rgb_list = np.array(self.get_values('rgb'))
        rgb_list = rgb_list.reshape(
            (rgb_list.shape[0], 1, rgb_list.shape[1])).astype('uint8')
        if repeat:
            bulked_up = np.repeat(
                np.repeat(np.moveaxis(rgb_list, 0, 1), width, 0), width, 1)
        else:
            bulked_up = np.repeat(np.moveaxis(rgb_list, 0, 1), width, 0)

        if add_white_line:
            white_width = 5
            for i in reversed(range(1, self.length())):
                bulked_up = np.insert(
                    bulked_up, [i * width for k in range(white_width)], [255], axis=1)
        # 返回用于生成图片的array
        return bulked_up
    ...
    def change_partial_color_with_random(self, random_count):
        for color_index in range(self.length() - 1, max(0, self.length() - random_count), -1):
            while True:
                ...
                cur_colors = [color.RGB_value(is_geo=False) for index, color in enumerate(self.colors) if
                              index != color_index]
