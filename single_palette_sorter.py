...
from mlrose.fitness import *
from mlrose.algorithms import *
from mlrose.opt_probs import *
...
class SinglePaletteSorter:
    ...
    def _tsp_sort(self, lab_distance_mode, tsp_solver_mode, merge_cut_type=MergeCutType.Without_Cutting):
        ...
        reordered_best_state = reordered_best_state[
                               reordered_best_state.index(initial_node_index):] + reordered_best_state[
                                                                                  :reordered_best_state.index(
                                                                                      initial_node_index)]
    ...
    def _nearest_neighbor_tsp(self, distance_matrix):
        ...
        for initial_color_index in range(self.palette.length()):
            ...
            distance = sum(
                [distance_matrix[tour[i % num_points]][tour[(i + 1) % num_points]] for i in range(num_points)])
    ...
    def _genetic_tsp(self, distance_matrix):
        ...
        ga_tsp = GA_TSP(func=_genetic_total_distance, n_dim=self.palette.length(), size_pop=50, max_iter=100,
                        prob_mut=1)
    ...
    def _simulated_annealing_tsp(self, distance_matrix):
        ...
        sa_tsp = SA_TSP(func=_simulated_annealing_total_distance, x0=range(self.palette.length()), T_max=T_max,
                        T_min=T_min, L=iteration_per_temperature)
    ...
    def _ant_colony_tsp(self, distance_matrix, ant_count=50):
        ...
        aca = ACA_TSP(func=_ant_colony_total_distance, n_dim=self.palette.length(), size_pop=ant_count, max_iter=100,
    ...
    def _find_farthest_neighbor_index(self, distance_matrix, tour):
        ...
        for color_index in tour:
            distances = [(index, distance) for index, distance in enumerate(distance_matrix[color_index]) if
                         index not in tour]
    ...
    def _find_closest_edge(self, distance_matrix, tour, farthest_neighbor_index):
        ...
        for current_tour_index in range(len(tour)):
            ...
            new_sub_tour_distance = distance_matrix[tour[current_tour_index]][farthest_neighbor_index] + \
                                    distance_matrix[farthest_neighbor_index][tour[next_tour_index]]
    ...
    def _TSP_cut_palette_largest(self, TSP_graph_indices, distance_matrix, merge_cut_type):
        ...
        largest_palette_start, largest_palette_end = self._find_largest_palette_indices(TSP_graph_indices,
                                                                                        distance_matrix)
        # Decide the target: inner or outer colors
        smaller_boundary = min(TSP_graph_indices.index(largest_palette_start),
                               TSP_graph_indices.index(largest_palette_end))
        larger_boundary = max(TSP_graph_indices.index(largest_palette_start),
                              TSP_graph_indices.index(largest_palette_end))

        is_outer, target_colors = self._find_colors_inside_boundary(TSP_graph_indices, larger_boundary,
                                                                    smaller_boundary)
        ...
        if len(target_colors) == 0:
            ...
            if is_outer:
                start_index, end_index = self._find_outer_case_start_end_indices(TSP_graph_indices, target_colors,
                                                                                 distance_matrix, larger_boundary,
                                                                                 smaller_boundary, merge_cut_type)
            else:
                start_index, end_index = self._find_inner_case_start_end_indices(TSP_graph_indices, target_colors,
                                                                                 distance_matrix, larger_boundary,
                                                                                 smaller_boundary, merge_cut_type)
    ...
    def _find_outer_case_start_end_indices(self, TSP_graph_indices, target_colors, distance_matrix, larger_boundary,
                                           smaller_boundary, merge_cut_type):
    ...
    def _find_inner_case_start_end_indices(self, TSP_graph_indices, target_colors, distance_matrix, larger_boundary,
                                           smaller_boundary, merge_cut_type):
        larger_index = TSP_graph_indices[larger_boundary]
        smaller_index = TSP_graph_indices[smaller_boundary]
        start_index = target_colors[-1]
